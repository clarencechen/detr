...
def box_iou(x1, x2):
    ...
    x1_exp, x2_exp = tf.expand_dims(x1, -2), tf.expand_dims(x2, -3)
    lt = tf.maximum(x1_exp[..., :2], x2_exp[..., :2])  # [...,N,M,2]
    rb = tf.minimum(x1_exp[..., 2:], x2_exp[..., 2:])  # [...,N,M,2]

    wh = tf.maximum(rb - lt, 0)  # [...,N,M,2]
    inter = wh[..., 0] * wh[..., 1]  # [...,N,M]

    union = tf.expand_dims(a1, -1) + tf.expand_dims(a2, -2) - inter
...
def generalized_box_iou(x1, x2):
    ...
    Returns a [..., N, M] pairwise matrix, where N = x1.shape[-1]
    and M = x2.shape[-1]
    """
    ...
    with tf.control_dependencies([x1_ok, x2_ok]):
        ...
        x1_exp, x2_exp = tf.expand_dims(x1, -2), tf.expand_dims(x2, -3)
        lt = tf.minimum(x1_exp[..., :2], x2_exp[..., :2])
        rb = tf.maximum(x1_exp[..., 2:], x2_exp[..., 2:])
